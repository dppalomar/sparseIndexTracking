...
X <- data_2010_2015[[1]]
SP500 <- data_2010_2015[[2]]
```
The file `data_2010_2015.RData` contains a list of two xts objects:
...
> 1. `X`: A $T\times N$ xts with the daily linear returns of the $N$ assets that were in the index during the period   2010-2015 (total $T$ trading days)
  2. `SP500`: A $T\times 1$ xts with the daily linear returns of the index S\&P 500 during the period 2010-2015

Note that we use xts objects just for illustration purposes. The function `spIndexTracking()` can be further used with simple data arrays or dataframes.
