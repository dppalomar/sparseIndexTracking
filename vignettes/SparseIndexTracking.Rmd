...
vignette: "%\\VignetteIndexEntry{Design of portfolio of stocks to track an index}
...
#rmarkdown::render("vignettes/SparseIndexTracking.Rmd", "bookdown::pdf_document2")
#rmarkdown::render("vignettes/SparseIndexTracking.Rmd", "bookdown::html_document2")
...
We refer to the iterative procedure of Proposition \@ref(prp:AS1) as AS$_{1}(\mathbf{q})$ (Active-Set for $u=1$).
...
```  
We refer to the iterative procedure of Proposition \@ref(prp:ASu) as AS$_u(\mathbf{q})$ (Active-Set for general $u<1$).
...
The initial point of the algorithm is set by default to $\mathbf{w}^{(0)} = \frac{1}{N}\mathbf{1}$, unless the user specifies otherwise. 
