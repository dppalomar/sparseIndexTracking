...
for (p in periods){
  for (tw in test_windows){
    for (rt in return_type){
      ...
      png(file=paste0(name, '.png'), width = 15, height = 10.5, units = "cm", res = 800)
      par(mfrow=c(2,1))
      plot(normalized_wealth, legend.loc = 'bottomleft', main = "Normalized P&L")
      plot(excessPnL, main = "Excess P&L")
      dev.off()

      pdf(paste0(name, '.pdf'), width = 15*0.393701, height = 10.5*0.393701)
      par(mfrow=c(2,1))
      plot(normalized_wealth, legend.loc = 'bottomleft', main = "Normalized P&L")
      plot(excessPnL, main = "Excess P&L")
      dev.off()
...
# load('per4_126_67_hub4_w.RData')
#
# png(file="per4_126_67_hub4_w.png", width = 15, height = 10.5, units = "cm", res = 800)
# par(mfrow=c(2,1))
# plot(normalized_wealth, legend.loc = 'bottomleft', main = "Normalized P&L")
# plot(excessPnL, main = "Excess P&L")
# dev.off()
#
# pdf('per4_126_67_hub4_w.pdf', width = 15*0.393701, height = 10.5*0.393701)
# par(mfrow=c(2,1))
# plot(normalized_wealth, legend.loc = 'bottomleft', main = "Normalized P&L")
# plot(excessPnL, main = "Excess P&L")
# dev.off()
